...
const SearchGroupAdd = ({onAddMember}) => {
  const uzer = useSelector(state => state?.user?.user)
  ...
  return (
    <div>
        <div  className='w-full dark:bg-slate-500 flex border px-1 mt-5 rounded-sm border-slate-400 justify-center items-center'>
            <input type="text" className='outline-none dark:text-slate-200 dark:bg-slate-500 p-2  w-full' value={phrase} onChange={(e)=> setPhrase(e.target.value)} placeholder='Search users to add' />
            <CiSearch className='text-[19px] dark:text-slate-100 cursor-pointer  font-semibold' />
        </div>
        <div className='mt-2 overflow-y-auto scrollbar h-40'>
            {
            ...
            }
            {
              search.length !== 0 && !loading && (
                search.map((user, index) => {
                  return(
                    <div key={user?._id} onClick={() => onAddMember(user)} className={`${user._id === uzer.id? "hidden":"block"} flex hover:border rounded-md hover:border-green-600 items-center gap-3 p-2`}>
                      <div>
